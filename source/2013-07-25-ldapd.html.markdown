...
A continuación cree cuentas de usuario, tenga en cuenta que en la configuración por defecto debe emplear cifrado SHA para la clave (que no es seguro pero es soportada por ```phpLdapAdmin``` y ```ldadpd```).

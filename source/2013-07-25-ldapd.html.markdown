...
```
Agregando la respectiva línea con schema en ```/etc/ldapd.conf```.  En el caso de cosine.schema tiene varias definciones repetidas respecto a otros así que deben comentarse, podrá determinar las líneas repetidas ejecutando iterativamente a medida que pone en comentarios:
<pre>
doas ldapd -n
</pre>
